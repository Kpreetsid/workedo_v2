		...
		setTasks([
			{
				...
				title: "",
				type: "Text",
				options: [], // THIS is for radio/checkbox options
			},
			...tasks
		]);
	};
		...
		setTasks(
			tasks.map((task: any) =>
				task.id === taskId
					? {
						...task,
						options: task.options.filter((t: any, i: any) => i !== index),
					}
					: task
			)
		);
				...
				task.type === "Check Box"
					...
					(
						task.type === "Radio Button"
							?
							task.options.map((opt: string, index: string) => ({
								key: opt,
								value: index,
							}))
							:
			...
			<ScrollView style={styles.taskContainer}>
				...
				{tasks.map((task: any) => (
					<View key={task.id} style={styles.taskCard}>
								...
								<View style={styles.dropdownMenu}>
									{DROPDOWN_OPTIONS.map((option) => (
										<Pressable
											...
											onPress={() => {
												console.log('updating task type to ', task.id, option);
						...
						{/* OPTIONS (only for Radio Button / Check Box) */}
