...
export default function UpdatePartInventoryModal({ part, visible, onClose, onSubmit }: UpdatePartInventoryModalProps) {
	const [current, setCurrent] = useState(part.quantity.toString());
	const [add, setAdd] = useState("");
	const [total, setTotal] = useState((Number(current) + Number(add)).toString());
				...
				<View style={styles.inputRow}>
					...
					<View style={styles.inputContainer}>
						...
						<TextInput
							...
							onChangeText={(qty)=>{
								setAdd(qty);
								setTotal((Number(current) + Number(qty)).toString());
							}}
							keyboardType="numeric"
							placeholder="0"
						/>
