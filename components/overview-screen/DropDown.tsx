...
export default function Dropdown({name, label, options, value, onValueChange, openDropdown, setOpenDropdown}: DropdownProps) {
        ...
        <View style={styles.dropdownWrapper}>
            <View style={[styles.dropdownBox, variant === "purple" ? styles.purpleBorder : styles.grayBorder]}>
                ...
                <TouchableOpacity style={styles.dropdownTouchable} activeOpacity={0.8} onPress={() => setOpenDropdown(isOpen ? null : name)}>
                    <Text numberOfLines={1} ellipsizeMode="tail" style={styles.dropdownText}>{selected.label}</Text>
                    <DropDownIcon/>
                </TouchableOpacity>
                ...
                {isOpen && (
                    <View style={styles.optionsContainer}>
                        ...
                        <ScrollView style={styles.scrollView} showsVerticalScrollIndicator={true} nestedScrollEnabled>
                            {options.map((option) => (
                                <TouchableOpacity key={option.value} style={styles.optionItem} onPress={() => {
                                        onValueChange(option.value);
                                        setOpenDropdown(null);
                                    }}>
                                    <View style={[styles.checkbox, {backgroundColor: option.value === selected.value ? '#742BDE' : undefined}]}>
                                        <TickMark/>
                                    </View>
