...
export const mapUserToAsset = async (assetId: string) => {
	const url = `${endpoints.asset.userToAssets}?assetId=${assetId}&populate=userId`;
	return await sendRequest('GET', url);
}

export const fetchAssetChildren = async (assetId: string) => {
	const url = `${endpoints.asset.assetChild}/${assetId}`;
	return await sendRequest('GET', url);
}
